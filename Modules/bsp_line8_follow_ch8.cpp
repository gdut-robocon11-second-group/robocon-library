...
HAL_StatusTypeDef
line_follower_8ch::read_line_mask(uint8_t &mask,
...
// 放进数组，一次性读
HAL_StatusTypeDef
line_follower_8ch::read_line_array(std::array<uint8_t, channel_count> &states,
                                   std::chrono::milliseconds timeout) {
  // 把states数组初始化为0
  for (auto &v : states) {
    v = 0;
...
// 读取单个通道的巡线识别结果
HAL_StatusTypeDef
line_follower_8ch::read_channel_line(uint8_t channel, uint8_t &detected,
                                     std::chrono::milliseconds timeout) {
  detected = 0;
...
// 读取单个通道的灰度值
HAL_StatusTypeDef
line_follower_8ch::read_channel_gray(uint8_t channel, uint16_t &gray,
                                     std::chrono::milliseconds timeout) {
  gray = 0;
...
// 读取单个通道的阈值
HAL_StatusTypeDef
line_follower_8ch::read_channel_threshold(uint8_t channel, uint16_t &threshold,
                                          std::chrono::milliseconds timeout) {
  threshold = 0;
  if (!valid_channel(channel) || !is_i2c_mode()) {
    return HAL_ERROR;
...
// 读取所有灰度值
HAL_StatusTypeDef
line_follower_8ch::read_all_gray(std::array<uint16_t, channel_count> &gray,
                                 std::chrono::milliseconds timeout) {
  if (!is_i2c_mode()) {
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef
line_follower_8ch::read_snapshot(snapshot_gpio &data, bool with_gray,
                                 bool with_threshold,
                                 std::chrono::milliseconds timeout) {
...
HAL_StatusTypeDef
line_follower_8ch::read_snapshot(snapshot_i2c &data, bool with_gray,
                                 bool with_threshold,
                                 std::chrono::milliseconds timeout) {
  HAL_StatusTypeDef status = read_line_array(data.line, timeout);
  if (status != HAL_OK) {
...
HAL_StatusTypeDef
line_follower_8ch::i2c_read_u8(uint8_t reg, uint8_t &value,
                               std::chrono::milliseconds timeout) {
  if (m_i2c == nullptr) {
    return HAL_ERROR;
  }
...
// 读取一个16位的数据
HAL_StatusTypeDef
line_follower_8ch::i2c_read_u16_le(uint8_t reg, uint16_t &value,
                                   std::chrono::milliseconds timeout) {
  if (m_i2c == nullptr) {
    return HAL_ERROR;
  }
  ...
  uint8_t raw[2] = {0, 0};
  HAL_StatusTypeDef status = m_i2c->mem_read(
      m_i2c_hal_addr, reg, I2C_MEMADD_SIZE_8BIT, raw, 2, timeout);
  if (status != HAL_OK) {
    return status;
  }
