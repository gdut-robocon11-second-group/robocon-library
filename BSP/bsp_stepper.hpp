...
public:
  ...
  [[nodiscard]] static bool validate_response(const received_packet &packet,
    ...
    if (packet.register_address != (expected_register << 1)) {
      return false;
    }
    if (!check_crc(packet)) {
      return false;
  ...
  static constexpr uint8_t GCONF_REG_ADDR = 0x00;    // 全局配置寄存器地址
  static constexpr uint8_t GSTAT_REG_ADDR = 0x01;    // 全局状态寄存器地址
  static constexpr uint8_t IFCNT_REG_ADDR = 0x02;    // 接口计数寄存器地址
  static constexpr uint8_t NODECONF_REG_ADDR = 0x03; // 节点配置寄存器地址
  static constexpr uint8_t OTP_PROG_REG_ADDR = 0x04; // OTP编程寄存器地址
  static constexpr uint8_t OTP_READ_REG_ADDR = 0x05; // OTP读取寄存器地址
  static constexpr uint8_t IOIN_REG_ADDR = 0x06;     // 输入引脚状态寄存器地址
  static constexpr uint8_t FACTORY_CONF_REG_ADDR = 0x07; // 出厂配置寄存器地址

  // Velocity Dependent Control
  static constexpr uint8_t IHOLD_IRUN_REG_ADDR = 0x10; // 电流配置寄存器地址
  static constexpr uint8_t TPOWERDOWN_REG_ADDR = 0x11; // 电源管理寄存器地址
  static constexpr uint8_t TSTEP_REG_ADDR = 0x12;      // 步进时间寄存器地址
  static constexpr uint8_t TPWMTHRS_REG_ADDR = 0x13;   // PWM阈值寄存器地址
  static constexpr uint8_t VACTUAL_REG_ADDR = 0x22;    // 实际速度寄存器地址

  // StallGuard Control
  static constexpr uint8_t TCOOLTHRS_REG_ADDR = 0x14; // TCOOLTHRS寄存器地址
  static constexpr uint8_t SGTHRS_REG_ADDR = 0x40;    // SGTHRS寄存器地址
  static constexpr uint8_t SG_RESULT_REG_ADDR = 0x41; // SG_RESULT寄存器地址
