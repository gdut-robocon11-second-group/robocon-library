...
public:
  ...
  using addr_callback_t =
      gdut::function<void(uint16_t addr, uint32_t direction)>;
  ...
  // 初始化
  HAL_StatusTypeDef init() { return m_hi2c ? HAL_I2C_Init(m_hi2c) : HAL_ERROR; }
  ...
  HAL_StatusTypeDef transmit(
    ...
    return m_hi2c ? HAL_I2C_Master_Transmit(m_hi2c, dev_addr,
                                            const_cast<uint8_t *>(data), size,
                                            timeout_cast(timeout))
                  : HAL_ERROR;
  }

  // 阻塞模式接收
  HAL_StatusTypeDef receive(
      uint16_t dev_addr, uint8_t *data, uint16_t size,
      std::chrono::milliseconds timeout = std::chrono::milliseconds::max()) {
  ...
  // 中断模式发送
  HAL_StatusTypeDef transmit_it(uint16_t dev_addr, const uint8_t *data,
                                uint16_t size) {
    return m_hi2c ? HAL_I2C_Master_Transmit_IT(
                        m_hi2c, dev_addr, const_cast<uint8_t *>(data), size)
                  : HAL_ERROR;
  }

  // 中断模式接收
  HAL_StatusTypeDef receive_it(uint16_t dev_addr, uint8_t *data,
                               uint16_t size) {
    return m_hi2c ? HAL_I2C_Master_Receive_IT(m_hi2c, dev_addr, data, size)
                  : HAL_ERROR;
  }

  // 存储器写（阻塞）
  HAL_StatusTypeDef mem_write(
      uint16_t dev_addr, uint16_t mem_addr, uint16_t mem_addr_size,
  ...
  // 存储器读（阻塞）
  HAL_StatusTypeDef mem_read(
      uint16_t dev_addr, uint16_t mem_addr, uint16_t mem_addr_size,
      uint8_t *data, uint16_t size,
      std::chrono::milliseconds timeout = std::chrono::milliseconds::max()) {
    return m_hi2c ? HAL_I2C_Mem_Read(m_hi2c, dev_addr, mem_addr, mem_addr_size,
                                     data, size, timeout_cast(timeout))
                  : HAL_ERROR;
  }

  // 存储器写（中断）
  HAL_StatusTypeDef mem_write_it(uint16_t dev_addr, uint16_t mem_addr,
                                 uint16_t mem_addr_size, const uint8_t *data,
                                 uint16_t size) {
    return m_hi2c
               ? HAL_I2C_Mem_Write_IT(m_hi2c, dev_addr, mem_addr, mem_addr_size,
                                      const_cast<uint8_t *>(data), size)
               : HAL_ERROR;
  }

  // 存储器读（中断）
  HAL_StatusTypeDef mem_read_it(uint16_t dev_addr, uint16_t mem_addr,
                                uint16_t mem_addr_size, uint8_t *data,
                                uint16_t size) {
    return m_hi2c ? HAL_I2C_Mem_Read_IT(m_hi2c, dev_addr, mem_addr,
                                        mem_addr_size, data, size)
                  : HAL_ERROR;
  }

  // 从模式发送（阻塞）
  HAL_StatusTypeDef slave_transmit(
      const uint8_t *data, uint16_t size,
      std::chrono::milliseconds timeout = std::chrono::milliseconds::max()) {
    return m_hi2c ? HAL_I2C_Slave_Transmit(m_hi2c, const_cast<uint8_t *>(data),
                                           size, timeout_cast(timeout))
                  : HAL_ERROR;
  }

  // 从模式接收（阻塞）
  HAL_StatusTypeDef slave_receive(
      uint8_t *data, uint16_t size,
      std::chrono::milliseconds timeout = std::chrono::milliseconds::max()) {
    return m_hi2c ? HAL_I2C_Slave_Receive(m_hi2c, data, size,
                                          timeout_cast(timeout))
                  : HAL_ERROR;
  }
  ...
  HAL_StatusTypeDef slave_transmit_it(const uint8_t *data, uint16_t size) {
    return m_hi2c ? HAL_I2C_Slave_Transmit_IT(m_hi2c,
                                              const_cast<uint8_t *>(data), size)
                  : HAL_ERROR;
  }
  ...
  // 检查设备是否就绪
  HAL_StatusTypeDef is_device_ready(
      uint16_t dev_addr, uint32_t trials = 3,
      std::chrono::milliseconds timeout = std::chrono::milliseconds(10)) {
    return m_hi2c ? HAL_I2C_IsDeviceReady(m_hi2c, dev_addr, trials,
                                          timeout_cast(timeout))
                  : HAL_ERROR;
  }
...
public:
  ...
  // 地址匹配中断
  static void handle_addr(I2C_TypeDef *instance, uint16_t addr,
                          uint32_t direction) {
    uint8_t idx = get_i2c_index(instance);
    if (idx < 3 && m_i2cs[idx]) {
